...
static thread* createThreadsArr(int num_of_threads) {
    ...
    for(int i = 0 ; i < num_of_threads ; i++) {
        ...
        args->thread_id = i;
        pthread_create(&(thread_pool->threads[i].thread), NULL ,thread_handle_request, (void*)handle_func_args);
    }
    return new_arr
}
...
thread_pool* create_new_pool(int num_of_threads) {
    ...
    thread_pool->threads = createThreadsArr(num_of_threads); 
    return thread_pool;
}
...
void* thread_handle_request(void* args) {
    ...
    thread_pool* thread_pool = curr_args->thread_pool;
